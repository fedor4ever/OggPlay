...
int CMadDecoder::Open(RFile* f, const TDesC& /* aFilename */)
{
...
//  Clear();
  ...
  int fd   = 0; // wopen((wchar_t*)aFilename.Ptr(), O_RDONLY | O_BINARY);   
  /* struct stat buf;
  ...
  if(fd) filetag = 0; // id3_file_fdopen(fd, ID3_FILE_MODE_READONLY);
  return 0;
}

int CMadDecoder::OpenInfo(RFile* f, const TDesC& aFilename)
{
...
TInt CMadDecoder::Read(TDes8& aBuffer, TInt Pos) 
{
...
int CMadDecoder::mad_read(unsigned char* outputbuffer,int length)
{
  ...
  { 
    ...
    if(iStream.buffer==NULL || iStream.error==MAD_ERROR_BUFLEN)
    {
       ...
       * left untouched.
       */
      ReadSize= 0 ; // BstdRead(ReadStart,1,ReadSize,iBstdFile);
      if(ReadSize<=0)
      {
        /* if(ferror(iInputFp))
        {
...
void CMadDecoder::Id3GetString(TDes& aString,char const *id)
{
  ...
  else if(filetag) { tagtouse= 0; /* id3_file_tag(filetag); */ } //ID3V1
  else return; 

  frame = 0; // id3_tag_findframe(tagtouse,id,0);
  if (frame) 
  {
    field = 0; // id3_frame_field(frame, 1);
    ucs4 = 0; // id3_field_getstrings(field, 0);
    /* if (strcmp(id, ID3_FRAME_GENRE) == 0)
		ucs4 = id3_genre_name(ucs4); */

	if(ucs4)
    {
      utf8= 0; // id3_ucs4_utf8duplicate(ucs4);
...
//  struct id3_tag *tag = 0;
  ...
  else
  {
    ...
    while (count < tagsize) {
      int len;
      do
        len = 0 ; // BstdRead(allocated + count, 1,tagsize - count, iBstdFile  );
      while (len == -1 );
  ...
  id3tag = 0; // id3_tag_parse(data, tagsize); //FXIMAD: leak?
...
void CMadDecoder::MadHandleError() 
{
  ...
  if(iStream.error== MAD_ERROR_LOSTSYNC) {
    tagsize = 0; // id3_tag_query(iStream.this_frame,iStream.bufend - iStream.this_frame);
    if (tagsize > 0) {
...
void CMadDecoder::process_id3(struct id3_tag const *tag)
{
  ...
  // length information
  frame = 0; // id3_tag_findframe(tag, "TLEN", 0);
  if (frame) {
    union id3_field const *field;
    unsigned int nstrings;
    
    field    = 0; // id3_frame_field(frame, 1);
    nstrings = 0; // id3_field_getnstrings(field);
    
    if (nstrings > 0) {
      id3_latin1_t *latin1;
      
      latin1 = 0; // id3_ucs4_latin1duplicate(id3_field_getstrings(field, 0));
      if (latin1) {
        signed long ms;
...
//    !(player->options & PLAYER_OPTION_IGNOREVOLADJ)) {
  if(0) {
    frame = 0; // id3_tag_findframe(tag, "RVA2", 0);
    if (frame) {
      ...
      id   = 0; // id3_field_getlatin1(id3_frame_field(frame, 0));
      data = 0; // id3_field_getbinarydata(id3_frame_field(frame, 1), &length);
